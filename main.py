...
def compare_files(path1:str, path2:str, column):
    ...
    # This approach is significantly more performant than converting to sets.
    # .isin() is a vectorized operation that is highly optimized in pandas.
    # We get the unique values from the second dataframe, then use the `isin`
    # method with a negation (~) to filter the first dataframe for values
    # that are *not* in the second. Finally, we get the unique values from
    # the result to match the original function's behavior.
    df2_uniques = df2[column].unique()
    data_in_df1_not_in_df2 = df1[~df1[column].isin(df2_uniques)][column].unique()
