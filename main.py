...
def compare_files(path1:str, path2:str, column):
    ...
    # For optimal performance, we use pandas' vectorized `isin` method,
    # which is significantly faster than converting to sets for large datasets.
    df2_uniques = df2[column].unique()

    # The '~' operator inverts the boolean mask, selecting elements from
    # df1 that are NOT in df2_uniques. We call .unique() again to ensure
    # the final result contains only unique values.
