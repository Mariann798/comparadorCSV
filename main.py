...
def compare_files(path1:str, path2:str, column):
    ...
    # Using .unique() followed by vectorized .isin() is significantly faster than
    # Python set difference for large datasets. This approach leverages pandas'
    # optimized routines for membership testing while keeping memory usage low
    # by only operating on unique values.
    u1 = df1[column].unique()
    u2 = df2[column].unique()
    data_in_df1_not_in_df2 = u1[~pd.Series(u1).isin(u2)]
