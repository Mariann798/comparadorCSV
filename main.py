...
def compare_files(path1:str, path2:str, column):
    ...
    # Optimization: Use pandas vectorized isin operation on unique values.
    # Vectorized operations in pandas are implemented in C and are significantly
    # faster than Python set subtraction for large datasets.
    # This approach also avoids the overhead of converting NumPy arrays to Python sets.
    unique1 = df1[column].unique()
    unique2 = df2[column].unique()

    mask = ~pd.Series(unique1).isin(unique2)
    data_in_df1_not_in_df2 = unique1[mask]
